...
#define PNETPROTOTXT "/home/pouya/PycharmProjects/CNNbasedFaceDetector/model/det1.prototxt"
#define PNETWEIGHT "/home/pouya/PycharmProjects/CNNbasedFaceDetector/model/det1.caffemodel"

#define RNETPROTOTXT "/home/pouya/PycharmProjects/CNNbasedFaceDetector/model/det2.prototxt"
#define RNETWEIGHT "/home/pouya/PycharmProjects/CNNbasedFaceDetector/model/det2.caffemodel"

#define ONETPROTOTXT "/home/pouya/PycharmProjects/CNNbasedFaceDetector/model/det3.prototxt"
#define ONETWEIGHT "/home/pouya/PycharmProjects/CNNbasedFaceDetector/model/det3.caffemodel"
...
//    create scale pyramid
    ...
    if(numbox>0) {
        ...
        cv::Mat dy = pad_out.at(0) ;
        cv::Mat edy =  pad_out.at(1) ;
        cv::Mat dx = pad_out.at(2) ;
        cv::Mat edx =  pad_out.at(3) ;
        cv::Mat y =  pad_out.at(4) ;
        cv::Mat ey =  pad_out.at(5) ;
        cv::Mat x =  pad_out.at(6) ;
        cv::Mat ex =  pad_out.at(7) ;
        cv::Mat tmpw =  pad_out.at(8) ;
        cv::Mat tmph =  pad_out.at(9) ;
...
{
//    std::string file_name = "/home/pouya/PycharmProjects/CNNbasedFaceDetector/images/1.jpg" ;
//    cv::Mat img = cv::imread(file_name.c_str() ) ;
    init_nets(PNETPROTOTXT,PNETWEIGHT,RNETPROTOTXT,RNETWEIGHT,ONETPROTOTXT,ONETWEIGHT);
    nvx::Timer timer;
    double time ;
...
//    cap >> frame;
    ...
    if( true )
    {
      while( true )
      {
        ...
        {
            timer.tic() ;
            cv::vector<cv::Mat> res =  detecte_faces(frame) ;
            time = timer.toc() ;
            drawBoxes(frame,res[0]);
            cv::imshow("res",frame) ;
            std::cout << "Detection Time: " << time << " ms / "
                    << 1000.0 / time << " FPS" << std::endl;
        }
        else
...
//    cv::vector<cv::Mat> res =  detecte_faces(img) ;
//    drawBoxes(img,res[0]);
//    cv::imshow("res",img) ;
//    cv::waitKey(0);
